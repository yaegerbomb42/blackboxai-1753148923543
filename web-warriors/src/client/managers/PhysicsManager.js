...
export class PhysicsManager {
  ...
  createRoomBoundaries() {
    ...
    // North wall
    const northWallShape = new CANNON.Box(new CANNON.Vec3(roomSize/2, wallHeight/2, wallThickness/2));
    ...
    // South wall
    const southWallShape = new CANNON.Box(new CANNON.Vec3(roomSize/2, wallHeight/2, wallThickness/2));
    ...
    // East wall
    const eastWallShape = new CANNON.Box(new CANNON.Vec3(wallThickness/2, wallHeight/2, roomSize/2));
    ...
    // West wall
    const westWallShape = new CANNON.Box(new CANNON.Vec3(wallThickness/2, wallHeight/2, roomSize/2));
  ...
  sphereCast(from, to, radius) {
    ...
    for (let i = 0; i <= distance; i += radius) {
      ...
      for (const body of this.bodies) {
        const bodyPos = new CANNON.Vec3(body.position.x, body.position.y, body.position.z);
