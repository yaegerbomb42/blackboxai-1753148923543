...
export class WebManager {
  ...
  createWeb(startPosition, endPosition, spider) {
    ...
    web.constraint = this.createWebConstraint(
      startPosition,
      endPosition,
      spider
    );
  ...
  checkWebIntegrity(web) {
    ...
    try {
      ...
      const validIntersections = intersections.filter((intersection) => {
        return (
          intersection.object !== web.spider.mesh &&
          intersection.object !== web.line &&
          intersection.distance < distance - 0.5
        ); // Small buffer
      });
  ...
  distanceToLine(point, lineStart, lineEnd) {
    ...
    const t = Math.max(
      0,
      Math.min(1, pointToStart.dot(line) / (lineLength * lineLength))
    );
  ...
  shootWeb(spider, direction, maxDistance = 10) {
    try {
      ...
      const validIntersections = intersections.filter((intersection) => {
        return (
          intersection.object !== spider.mesh &&
      ...
      if (validIntersections.length > 0) {
        ...
        console.log(
          'Web attached to surface at distance:',
          validIntersections[0].distance.toFixed(2)
        );
      } else {
        // No valid attachment point found, use max distance
        endPos = startPos
          .clone()
          .add(direction.clone().multiplyScalar(maxDistance));
